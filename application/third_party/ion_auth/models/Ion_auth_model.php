...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password_by_ID($identity, $old, $new)
	{
		$query = $this->db->select('id, password, salt')
		                  ->where('id', $identity)
		                  ->limit(1)
		...
		if ($old_password_matches === TRUE)
		{
			...
			{
				$this->trigger_events(array('post_change_password', 'post_change_password_unsuccessful'));
				return array('status' => FALSE,'message' => "Old and New same password.");
			}
		}else{
			return array('status' => FALSE,'message' => 'Old password not matched.');
		}

		return array('status' => FALSE,'message' => 'Something wrong. Password has not changed. Try again later.');
	}
